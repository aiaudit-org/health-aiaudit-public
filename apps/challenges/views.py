...
def create_challenge_using_zip_file(request, challenge_host_team_pk):
    ...
    try:
        ...
        logging.info("####################################")
        logging.info(uploaded_zip_file.pk)
        if zip_config:
            ...
            if challenge.is_docker_based:
                ...
                logging.info(challenge_evaluation_cluster)
                #AI4H BUG
                ...
                logging.info(evaluation_cluster_serializer.is_valid())
...
def invite_users_to_challenge(request, challenge_pk):
    ...
    if not is_user_a_host_of_challenge(request.user, challenge.pk) and not request.user.is_superuser:
        response_data = {
...
# AI4H: Questionnaire  POST view
@api_view(["POST"])
@throttle_classes([UserRateThrottle])
@permission_classes((permissions.IsAuthenticated, HasVerifiedEmail))
@authentication_classes((JWTAuthentication, ExpiringTokenAuthentication))
def challenge_questionnaire_submit(request):
    """
        POSTS a questionnaire
        Returns:
            {dict} -- Participant team detail that has participated in the challenge
    """

    try:
        questionnaire = ChallengeQuestionnaire.objects.filter(challenge_id=request.data.get("challenge_id"),team_id=request.data.get("team_id")).first()
        serializer = QuestionnaireSerializer(
            data=request.data, context={"request": request}
        )
        if questionnaire and serializer.is_valid():
            #remove all elemnts in dict that have empty string
            d = {k: v for k, v in request.data.items() if v}
            #partially update existing Questionnaire
            for i in d:
                setattr(questionnaire, i, d[i])
            questionnaire.save()

            #get serializer with updated data
            serializer = QuestionnaireSerializer(
                questionnaire, context={"request": request}
            )
            response_data = serializer.data
            return Response(response_data, status=status.HTTP_200_OK)

        elif not questionnaire and serializer.is_valid():
            serializer.save()
            response_data = serializer.data
        return Response(response_data, status=status.HTTP_201_CREATED)
    except Exception as e:
...
# AI4H: Questionnaire view
@api_view(["GET"])
@throttle_classes([UserRateThrottle])
@permission_classes((permissions.IsAuthenticated, HasVerifiedEmail))
@authentication_classes((JWTAuthentication, ExpiringTokenAuthentication))
def get_challenge_questionnaire(request,challenge_id,team_id):
    """
        Gets questionnaire
        Returns:
            {dict} -- Participant team detail that has participated in the challenge
    ...
    try:
        questionnaire = ChallengeQuestionnaire.objects.get(challenge_id=challenge_id,team_id=team_id)
    except ChallengeQuestionnaire.DoesNotExist:
        response_data = {}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
