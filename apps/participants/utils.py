from challenges.models import Challenge, ChallengeQuestionnaire
...
#AI4H
def user_challenge_questionnaire(user, challenge_id):
    """Returns questionnaire if the user has filled out questionnaire"""
    participant_teams = get_participant_teams_for_user(user)
    print(participant_teams)
    return ChallengeQuestionnaire.objects.filter(
        challenge_id=challenge_id, team_id=participant_teams
    )
