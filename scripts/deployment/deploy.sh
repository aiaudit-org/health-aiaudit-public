...
case $opt in
        ...
        pull)
            ...
            #adapted for ai4h
            aws s3 cp s3://evalai-secrets/evalai/${env}/docker/docker_${env}.env ./docker/prod/docker_${env}.env
        ...
        deploy-worker)
            ...
            if [ ${env} == "staging" ]; then
                queue_name=$(curl -k -L -X GET -H "Authorization: Token $token" https://staging.health.aiaudit.org/api/challenges/get_broker_url/$challenge/)
            elif [ ${env} == "production" ]; then
                queue_name=$(curl -k -L -X GET -H "Authorization: Token $token" https://health.aiaudit.org/api/challenges/get_broker_url/$challenge/)
            fi
        ...
        deploy-workers)
            ...
            if [ ${env} == "staging" ]; then
                queue_names=$(curl -k -L -X GET -H "Authorization: Token $token" https://staging.health.aiaudit.org/api/challenges/get_broker_urls/)
            elif [ ${env} == "production" ]; then
                queue_names=$(curl -k -L -X GET -H "Authorization: Token $token" https://health.aiaudit.org/api/challenges/get_broker_urls/)
            fi
