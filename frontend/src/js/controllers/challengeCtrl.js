...
(function() {
    ...
    function ChallengeCtrl(utilities, loaderService, $scope, $state, $http, $stateParams, $rootScope, $interval, $mdDialog, moment, $location, $anchorScroll, $timeout) {
        ...
        vm.acceptTermsAndConditions = function (acceptTermsAndConditionsForm) {
         ...
         // AI4h
        vm.submitQuestionnaire = function() {
            if (vm.isParticipated) {
              var parameters = {};

              vm.getTeamName(vm.challengeId);
              parameters.url = 'challenges/questionnaire/submission/';
              parameters.method = 'POST';
              if(vm.a_29 != ""){
                vm.a_29.join();
              }
              parameters.data = {
                  "challenge_id": vm.challengeId,
                  "team_id": vm.participated_team_name,
                  "a_1": vm.a_1,
                  "a_2": vm.a_2,
                  "a_3": vm.a_3,
                  "a_4": vm.a_4,
              ...
              parameters.token = vm.authToken;

              parameters.callback = {
                  onSuccess: function(response) {
                        var status = response.status;
                        if(status == 200){
                          $rootScope.notify("success", "Questionnaire has been submitted.");
                        }
                    },
                  onError: function(response) {
                    var status = response.status;
                    if(status == 406){
                      $rootScope.notify("error", "You have already submitted a questionnaire.");
                    }
                    else{
                      $rootScope.notify("error", "Questionnaire could not be submitted.");
                    }
        ...
        // AI4H - Get questionnaire
        vm.has_questionnaire = function() {
            parameters.url = 'challenges/' +vm.challengeId+ '/'+vm.participated_team_name+'/questionnaire/';
            parameters.method = 'GET';
            parameters.token = vm.authToken;
            parameters.callback = {
                onSuccess: function(response) {
